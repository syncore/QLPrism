...
// @name					QLPing, quakelive.com udp ping utility
// @version				1.0
...
if (!GM_updatingEnabled) {
  var AutoUpdater_171037={id:171037,days:1,name:SCRIPT_NAME,version:SCRIPT_VER,time:new Date().getTime(),call:function(response,secure){GM_xmlhttpRequest({method:"GET",url:"http"+(secure?"s":"")+"://userscripts.org/scripts/source/"+this.id+".meta.js",onload:function(xpr){AutoUpdater_171037.compare(xpr,response)},onerror:function(xpr){if(secure){AutoUpdater_171037.call(response,false)}}})},enable:function(){GM_registerMenuCommand(this.name+": Enable updates",function(){GM_setValue("updated_171037",new Date().getTime()+"");AutoUpdater_171037.call(true,true)})},compareVersion:function(r_version,l_version){var r_parts=r_version.split("."),l_parts=l_version.split("."),r_len=r_parts.length,l_len=l_parts.length,r=l=0;for(var i=0,len=(r_len>l_len?r_len:l_len);i<len&&r==l;++i){r=+(r_parts[i]||"0");l=+(l_parts[i]||"0")}return(r!==l)?r>l:false},compare:function(xpr,response){this.xversion=/\/\/\s*@version\s+(.+)\s*\n/i.exec(xpr.responseText);this.xname=/\/\/\s*@name\s+(.+)\s*\n/i.exec(xpr.responseText);if((this.xversion)&&(this.xname[1]==this.name)){this.xversion=this.xversion[1];this.xname=this.xname[1]}else{if((xpr.responseText.match("the page you requested doesn't exist"))||(this.xname[1]!=this.name)){GM_setValue("updated_171037","off")}return false}var updated=this.compareVersion(this.xversion,this.version);if(updated&&confirm("A new version of "+this.xname+" is available.\nDo you wish to install the latest version ("+this.xversion+")?")){var path="http://userscripts.org/scripts/source/"+this.id+".user.js";if(window.chrome){prompt("This script can't be updated automatically in Chrome.\nPlease uninstall the old version, and navigate to the URL provided below.",path)}else{try{window.parent.location.href=path}catch(e){}}}else{if(this.xversion&&updated){if(confirm("Do you want to turn off auto updating for this script?")){GM_setValue("updated_171037","off");this.enable();if(window.chrome){alert("You will need to reinstall this script to enable auto-updating.")}else{alert("Automatic updates can be re-enabled for this script from the User Script Commands submenu.")}}}else{if(response){alert("No updates available for "+this.name)}}}},check:function(){if(GM_getValue("updated_171037",0)=="off"){this.enable()}else{if(+this.time>(+GM_getValue("updated_171037",0)+1000*60*60*24*this.days)){GM_setValue("updated_171037",this.time+"");this.call(false,true)}GM_registerMenuCommand(this.name+": Check for updates",function(){GM_setValue("updated_171037",new Date().getTime()+"");AutoUpdater_171037.call(true,true)})}}};AutoUpdater_171037.check();
}
...
contentEval(function() {
	...
	function qlping_sortLocationsByPing(obj) {
		var arr = [];
		for (var prop in obj) {
		...
		arr.sort(function(a, b) {
			// ugly, yeah. but easier to understand
			if ((typeof a.value.ping === 'undefined' || !parseInt(a.value.ping)) 
				&& (typeof b.value.ping === 'undefined' || !parseInt(b.value.ping))
			) {
				return 0;
			} else if ((typeof a.value.ping === 'undefined' || !parseInt(a.value.ping)) 
				&& (typeof b.value.ping !== 'undefined' && parseInt(b.value.ping))
			) {
				return 1;
			} else if ((typeof a.value.ping !== 'undefined' && parseInt(a.value.ping))
				...
				return a.value.ping - b.value.ping; 
		});
			...
			if (quakelive.IsLoggedIn()) {
				...
				$("#qlping_locations").click(function() {
					...
					if($('#mod_sam').length > 0) { // start a match
						...
						$('#qlv_postlogin_matches .midGrayTxt').remove();
					...
					//$('.postlogin_nav').nextAll().remove();
					$('.postlogin_nav li.selected').removeClass('selected');
					$('.postlogin_nav li#qlping_locations').addClass('selected');
					$('table#listViewBrowser').append("<thead><tr><th>Location</th><th>Server IP</th><th>Ping, ms</th></tr></thead>");
					...
					$.map(servers, function(server) {
						...
						tRow.append(tCell);
						tCell = $("<td>").html(server.ping);
						if(server.host != "N/A" && loc.hasOwnProperty("qlping_servers")) {
							tCell.attr("title", "Last ping requests in ms: " + loc.qlping_servers[server.host].ping_arr.join(', '));
						}
						tRow.append(tCell);
	...
	quakelive.mod_friends.roster.AddContact = function (item) {
			...
			if(messages.length > 0) {
				...
				while (item = re_locs.exec(html)) {
					...
					while (item = re_loc.exec(str)) {
						...
						var location_img = $("<img class='location_flag' src='' alt='' title='' style='border:0; float:left' width='16' height='11' />");
						if(typeof loc_found !== 'undefined') {
							location_img.attr('src', quakelive.resource(loc_found.GetFlagIcon()));
							location_img.attr('alt', loc_found.countryAbbr);
							location_img.attr('title', loc_found.country);
						}
						newhtml += ["<tr>",
						"<td style='text-align: left;'>", 
							"<a href='#' onclick='qlping_goLocation(",loc_found.id,")'>",
							location_img.get(0).outerHTML, 
							"<b style='white-space:nowrap; display:inline-block; margin-top:5px;'>", 
							loc_found.location, "</b></a></td>",
				...
				var re_locs = /((\w+)\s?(\w+)?\s(\d+)‑(\d+)\sms\s?){1,}/ig;
				while (item = re_locs.exec(html)) {
					var str = item[0];
					var re_loc = /([\w\-]+)\s?([\w\-]+)?\s(\d+)‑(\d+)\sms/ig;
					var item = null;
					var newhtml = ["<table>",
						"<tr>",
						"<th style='font-weight:bold;'>Location</th>",
						"<th style='font-weight:bold;'>My Ping, ms</th>",
						"<th style='font-weight:bold;'>Your Ping, ms</th>",
						"</tr>"
					].join("");
					while (item = re_loc.exec(str)) {
						var loc_ping = parseInt(item[3]);
						...
						if(typeof locdb.locsByCountry[loc_found.countryAbbr] !== 'undefined' 
							...
							loctext += " " + loc_found.city.substr(0,4);
						}
						newhtml += ["<tr>",
						"<td style='text-align: left;'>", 
							"<a href='#' onclick='qlping_goLocation(",loc_found.id,")'>",
		...
		item.FriendsContextMenuHandler = function (action, el, pos) {
			...
			for(var i = 0, len = servers.length; i < len; i++) {
				if(i == 10) { //limit to first ten locations
					break;
				}
				...
				if(Object.keys(loc.qlping_servers).length > 1) { // more than one server in location
					loctext += " " + loc.qlping_avg + " ms "; // \n doesn't work and <br/> gets escaped
				} else {
					loctext += " " + server.ping + " ms "; // \n doesn't work and <br/> gets escaped
				}
				output += loctext;
			}
			if(output.length > 0) {
				contact.SendMessage("My nearest locations:");
			...
			if(contact.hasOwnProperty("qlping_servers") && contact.qlping_servers.length > 0) {
				...
				arr.sort(function(a, b) {
					...
					if(!a.hasOwnProperty('locid') && !b.hasOwnProperty('locid')) {
						...
						a.diff = Math.abs(locdb.locations[a.locid].qlping_avg - a.ping);
						//a.diff = Math.round(a.diff/10)*10; //round by 10
						b.diff = Math.abs(locdb.locations[b.locid].qlping_avg - b.ping);
				...
				for(var i = 0, len = arr.length; i < len; i++) {
					...
					if(loc.qlping_avg > 100 || arr[i].ping > 100) {
					...
					loctext += " " + loc.qlping_avg + "‑" + arr[i].ping + " ms "; // \n doesn't work and <br/> gets escaped
					output += loctext;
				}
				if(output.length > 0) {
					contact.SendMessage("‑‑‑");
	...
	OnCvarChanged = function(name, val, replicate) { // hello wn, you made it better!
		...
		case "_qlping_announce":
			...
			for(var i = 0, len = servers.length; i < len; i++) {
				...
				var color = "^4";
				if(loc.qlping_avg < 45) {
					color = "^2";
				} else if(loc.qlping_avg < 75) {
					color = "^3";
				} else if(loc.qlping_avg >= 75) { // to be sure that ping is numeric
					color = "^1";
				}
				loctext += " " + color + loc.qlping_avg + " ms";
				output.push(loctext);
			}
			...
			qz_instance.SendGameCommand(currentOut + " ^2[qlping] My nearest locations:;");
